                  ...
                  entreprises.map((entreprise) => (
                    <tr key={entreprise.id}>
                      ...
                      <td>
                        ...
                        >
                          {/* Affichage consistant des statuts */}
                          {!entreprise.status || entreprise.status === 'pending' ? 'En attente' : 
                           entreprise.status === 'accepte' || entreprise.status === 'accepté' ? 'Accepté' : 
                           entreprise.status === 'refuse' || entreprise.status === 'refusé' ? 'Refusé' : 
                           entreprise.status.charAt(0).toUpperCase() + entreprise.status.slice(1)}
                        </Badge>
                      </td>
                      <td>
                      ...
                      <td>
                        <div className="d-flex gap-1">
                          {/* Bouton Accepter - Actif uniquement pour entreprises en attente */}
                          {(!entreprise.status || entreprise.status === 'en attente') ? (
                            <Button
                              variant="success"
                              disabled={loading}
                            ...
                            <Button
                              ...
                              title={entreprise.status === 'accepte' || entreprise.status === 'accepté' ? 'Cette entreprise est déjà acceptée' : 
                                    entreprise.status === 'refuse' || entreprise.status === 'refusé' ? 'Cette entreprise est déjà refusée' : 
                                    `Cette entreprise est déjà ${entreprise.status}`}
                            >
                              <FaCheck />
                            </Button>
                          )}
                          
                          {/* Bouton Refuser - Actif uniquement pour entreprises en attente */}
                          {(!entreprise.status || entreprise.status === 'en attente') ? (
                            <Button
                              variant="danger"
                              disabled={loading}
